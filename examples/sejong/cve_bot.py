...
# Handle '/cvenumber'
@bot.message_handler(commands=['cvenumber'])
def send_cveinfo_number(message):
    number = message.text.replace('/cvenumber', '').strip()

    report = cve.search_by_number(number)
